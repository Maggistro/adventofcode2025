...
void Network::step(std::map<std::string, Server*>& servers, std::string stepTarget) {
    ...
    for (const auto& [connection, visitCount] : connections) {
        ...
        if (server->getConnections().size() == 1) {
            removeConnection({node1, node2}, &newConnections);
        ...
        for (const std::string& connection : server->getConnections()) {
            insertConnection({server->getName(), connection}, &newConnections);
        }
    }
...
void Network::removeConnection(std::pair<std::string, std::string> connection, std::map<std::pair<std::string, std::string>, int>* map) {
